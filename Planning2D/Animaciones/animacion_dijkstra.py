...
from Algoritmos.Algoritmos2D_Animacion import dijkstra
...
fig.canvas.set_window_title("Algoritmo Dijkstra")
...
# Llamada y configuración inicial del algoritmo Dijkstra
algoritmo_dijkstra = dijkstra.Dijkstra2D()
algoritmo_dijkstra.configurar(entorno=base, start=start, meta=meta)
...
# Título del gráfico
# ax.set_title("Algoritmo Dijkstra", fontweight="bold", loc="center")
    ...
    if etiqueta is not None:
        ...
        if etiqueta != "ruta":
            if nodo != meta and nodo != start:
                if etiqueta == "no visitado":   # Nodo no visitado
                    representacion[nodo] = [0, 128, 255]
                    cuadriculas.set_data(representacion)
                else:   # Nodo visitado
                    representacion[nodo] = [128, 0, 255]
                    cuadriculas.set_data(representacion)
                    # Indicando las distancias de cada cuadricula
                    # Para aumentar la velocidad de animación eliminar las siguientes líneas
                    # distancia = datos[2]
...
animacion = FuncAnimation(fig, update, frames=algoritmo_dijkstra.ejecutar(), interval=10, blit=False, repeat=False)
plt.show()
